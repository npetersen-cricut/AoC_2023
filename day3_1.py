...
# If you're interested, the docstring was generated by CodiumAI, which also generates tests. For what it does, it's very impressive and free.
def check_distance(matrix):
    """
    Extracts valid numbers from a matrix.

    Args:
        matrix (list of lists): The input matrix containing alphanumeric characters, symbols and dots.

    Returns:
        list: A list containing all the valid numbers (numbers touching a symbol) extracted from the matrix.
